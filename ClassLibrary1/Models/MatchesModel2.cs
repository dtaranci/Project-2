namespace ClassLibrary1.Models.Two
{
    using System;
    ...
    public partial class MatchesModel2
    {
        [JsonProperty("venue")]
        ...
        [JsonProperty("time")]
        [JsonConverter(typeof(TimeConverter))] //was missing from quicktype
        ...
        [JsonConverter(typeof(StageNameConverter))] //was missing from quicktype
        ...
        => $"{Venue} - {HomeTeamCountry} vs {AwayTeamCountry} ({Winner})";
    }
    ...
    public partial class Team
    {
        ...
        public static Team Parse2(string line)
        {
            string[] splitline = line.Split(DELIMITER);
            return new Team
            {
        ...
        public static string Write(Team teamtowrite)
        {
            return $"{teamtowrite.Country}{DELIMITER}{teamtowrite.Code}";
        }
        public static string Write2(Team teamtowrite)
        {
            return $"{teamtowrite.Country}";
        }
    }
    ...
    public partial class TeamEvent
    {
        ...
        [JsonConverter(typeof(TypeOfEventConverter))] //was missing from quicktype
    ...
    public partial class TeamStatistics
    {
        ...
        [JsonConverter(typeof(TacticsConverter))] //was missing from quicktype
    ...
    public partial class StartingEleven
    {
        ...
        public static string Write(StartingEleven player)
        {
            if (!string.IsNullOrEmpty(player.PicturePath))
            {
                return $"{player.Name}{DELIMITER}{player.Captain}{DELIMITER}{player.ShirtNumber}{DELIMITER}{player.Position}{DELIMITER}{player.PicturePath}";
            }
            return $"{player.Name}{DELIMITER}{player.Captain}{DELIMITER}{player.ShirtNumber}{DELIMITER}{player.Position}";
        }
        public static StartingEleven ParseOne(string input)
        {
            ...
            if (temp.Length == 5)
            {
                ...
                {
                    Name = temp[0],
                    Captain = bool.Parse(temp[1]),
                    ShirtNumber = int.Parse(temp[2]),
                    Position = (Position)Enum.Parse(typeof(Position), temp[3]),
                    PicturePath = temp[4]
                };
            }
            return new StartingEleven
       ...
       /* public static IList<StartingEleven> ParseAll(string input)
        {
            string[] temp = input.Split(DELIMITER);
            IList<StartingEleven> tempstor = new List<StartingEleven>();

            tempstor.Add(new StartingEleven
            {
                Name = temp[0],
                Captain = bool.Parse(temp[1]),
                ShirtNumber = int.Parse(temp[2]),
    ...
    public enum StageName { Final, FirstStage, MatchForThirdPlace, PlayOffForThirdPlace, QuarterFinals, RoundOf16, SemiFinals };

    public enum Status { Completed };

    public enum Time { FullTime };

    public enum Description { ClearNight, Cloudy, CloudyNight, PartlyCloudy, PartlyCloudyNight, Sunny };
